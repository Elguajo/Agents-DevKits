...
- **Условные специалисты:** на схеме показан только частый путь. `change-impact-analysis`, `data-storage-review`, `data-migration`, `concurrency-review`, `reliability-review` и `observability-review` подключаются, только если задача даёт их триггер, а два проектных аудита запускаются исключительно по явному запросу.
...
`qa` предоставляется отдельно в составе gstack, а не входит в библиотеку из 53 навыков этого репозитория. Его стоит запускать для браузерной QA feature-ветки с циклом «проверить → исправить → перепроверить»; если исправления вносить нельзя, используйте `qa-only`.
