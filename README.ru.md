...
`qa` предоставляется отдельно в составе gstack, а не входит в библиотеку из 48 навыков этого репозитория. Его стоит запускать для браузерной QA feature-ветки с циклом «проверить → исправить → перепроверить»; если исправления вносить нельзя, используйте `qa-only`.
