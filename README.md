...
`qa` is provided separately by gstack, rather than by this repository's 53-skill library. It is the appropriate choice when a feature branch needs browser-based QA with an iterative test → fix → verify loop; use `qa-only` when fixes must not be made.
