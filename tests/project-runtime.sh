...
python3 "$repo_root/project.py" route --task "show the roadmap status with checkboxes" | grep -q '"roadmap-status"'
echo "==> every model-invocable owner is reachable from a task description"
echo "==> adjacent owners stay separated by phrasing"
python3 "$repo_root/tests/project-runtime-routing.py" "$repo_root"
